...
import { GAME_TYPES, RANKS } from '@hago/shared';
...
usersRouter.get('/leaderboard', (req, res) => {
  ...
  if (gameType && GAME_TYPES.includes(gameType as any)) {
    const rows = db
      .prepare(
        `SELECT u.*, p.avatar_seed, p.avatar_style, p.frame_id, p.title_id, gs.rating AS grating, gs.wins AS gwins, gs.matches AS gmatches
